...
import { 
  ...
  MoreHorizontal, Heading1, Heading2, Copy, Edit3, BarChart2, Play
} from 'lucide-react';
import './Audience.css';
...
const ColorPicker = ({ value, onChange }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#202024', padding: 5, borderRadius: 6, border: '1px solid #323238', marginBottom:15 }}>
...
const InlineToolbar = ({ onFormat, onInsertVar }) => (
    <div style={{
        ...
        <div style={{width:1, background:'#323238', margin:'0 5px'}}></div>
        <select onChange={(e) => { if(e.target.value) onInsertVar(e.target.value); e.target.value=''; }} style={{background:'#121214', color:'white', border:'1px solid #323238', borderRadius:4, fontSize:12, padding:'2px 5px', cursor:'pointer'}}>
...
const EditableArea = ({ html, onChange, style, tagName = 'div' }) => {
    ...
    useEffect(() => { if (contentEditableRef.current && contentEditableRef.current.innerHTML !== html) { contentEditableRef.current.innerHTML = html; } }, [html]);
    const handleInput = (e) => { onChange(e.currentTarget.innerHTML); };
    const execCmd = (command, value = null) => { document.execCommand(command, false, value); if (contentEditableRef.current) onChange(contentEditableRef.current.innerHTML); };
    const insertTextAtCursor = (text) => { const selection = window.getSelection(); if (!selection.rangeCount) return; selection.deleteFromDocument(); selection.getRangeAt(0).insertNode(document.createTextNode(text)); if (contentEditableRef.current) onChange(contentEditableRef.current.innerHTML); };
    return (
        <div style={{position:'relative'}}>
            <div className="editable-wrapper">
                <InlineToolbar onFormat={execCmd} onInsertVar={insertTextAtCursor} />
                <div ref={contentEditableRef} contentEditable onInput={handleInput} style={{outline: 'none', minHeight: '1em', ...style}} className="editor-content" />
            </div>
        </div>
    );
};
...
const generateHTML = (blocks, subject, previewText, includeFooter = false, npsContext = null) => {
  ...
  safeBlocks.forEach(block => {
    ...
    switch (block.type) {
      case 'header':
        const headerImg = c.imageUrl ? `<img src="${c.imageUrl}" alt="Logo" style="max-width: 200px; height: auto; border: 0; display: block;">` : `<h2 style="margin: 0; color: ${s.color || '#333'}; font-family: sans-serif;">SEU LOGO</h2>`;
      ...
      case 'text':
        ...
        break;
      case 'imagetext':
        blockHTML = `<tr><td style="padding: ${s.padding}px; background-color: ${s.backgroundColor};"><table width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td width="40%" valign="middle" style="padding-right: 20px;"><img src="${c.url}" style="width: 100%; border-radius: 4px; display: block; object-fit: cover;" /></td><td width="60%" valign="middle" style="font-family: sans-serif; font-size: 14px; color: ${s.color}; line-height: 1.5;"><div style="font-size: 18px; margin-bottom:8px; color: ${s.titleColor || '#000'}; font-weight:bold;">${c.title}</div><div>${c.text}</div></td></tr></table></td></tr>`;
        break;
      case 'image':
      ...
      case 'nps':
        ...
        let btns = '';
        for(let i=0; i<=10; i++) { btns += `<a href="${base}/feedback/${cid}/${contactId}/${i}" style="display:inline-block; width:30px; height:30px; line-height:30px; background:#8257e6; color:white; text-align:center; margin:2px; text-decoration:none; border-radius:4px; font-family:sans-serif;">${i}</a>`; }
  ...
  const footerHTML = includeFooter ? `<tr><td align="center" style="padding:20px; color:#999; font-family:sans-serif; font-size:11px; border-top:1px solid #eee;"><p>Enviado via Framework Workspace</p></td></tr>` : '';
...
export default function Announcements() {
  ...
  // --- FORÇAR ENVIO (BOTÃO PLAY) ---
  const handleForceSend = async (camp) => {
      if(!window.confirm(`Tem certeza que deseja enviar "${camp.title}" AGORA?`)) return;
      setLoading(true);
      try {
          const html = generateHTML(camp.content, camp.title, camp.preview_text, true);
          const { error } = await supabase.functions.invoke('send-email', {
              body: { 
                  type: 'campaign', 
                  campaignId: camp.id,
                  subject: camp.title, // GARANTIDO: Pega direto do banco, sem [TESTE]
                  html: html,
          ...
          await supabase.from('announcements').update({ status: 'sent', updated_at: new Date() }).eq('id', camp.id);
  ...
  const handleSendTest = async () => {
      ...
      setLoading(true);
      try {
          // Aqui no teste, a gente coloca [TESTE], mas isso não salva no banco
  ...
  const handleImageUpload = async (e, blockId, field = 'url') => { const file = e.target.files[0]; if (!file) return; try { const fileExt = file.name.split('.').pop(); const fileName = `${Date.now()}.${fileExt}`; const { error: uploadError } = await supabase.storage.from('campaign-images').upload(fileName, file); if (uploadError) throw uploadError; const { data } = supabase.storage.from('campaign-images').getPublicUrl(fileName); updateBlock(blockId, field, data.publicUrl); } catch (err) { alert("Erro no upload: " + err.message); } };

  // --- SAVE ---
  const handleSave = async (status = 'draft') => {
    if (!editorState.subject) return alert("Adicione um assunto!");
    setLoading(true);
    const payload = { title: editorState.subject, content: editorState.blocks, preview_text: editorState.previewText, audience_type: editorState.audienceType, tags: editorState.tags, status: status, updated_at: new Date() };
    if (status === 'scheduled') { if(!scheduleDate) return alert("Selecione data/hora."); payload.scheduled_at = new Date(scheduleDate).toISOString(); }
    try {
        let savedId = editorState.id;
        if (editorState.id) { await supabase.from('announcements').update(payload).eq('id', editorState.id); } else { const {data} = await supabase.from('announcements').insert([payload]).select(); savedId = data[0].id; }
        
        if(status === 'sent') {
            const html = generateHTML(editorState.blocks, editorState.subject, editorState.previewText, true);
            await supabase.functions.invoke('send-email', { body: { type: 'campaign', campaignId: savedId, subject: editorState.subject, html: html, audienceType: editorState.audienceType, tags: editorState.tags, baseUrl: window.location.origin } });
            alert("Enviado com sucesso! 🚀");
        } else if(status === 'scheduled') { 
            alert("Agendado! (Para forçar envio, use o botão Play na lista)"); 
            setIsScheduleModalOpen(false); 
        } else { alert("Rascunho salvo!"); }
        setViewMode('list'); fetchCampaigns();
            ...
            <tbody>
              {campaigns.map(camp => (
                <tr key={camp.id} style={{borderBottom:'1px solid #29292e'}}>
                  ...
                  <td>{camp.title}</td>
                  <td style={{color:'#a8a8b3'}}>
                      {camp.status === 'scheduled' ? `📅 ${new Date(camp.scheduled_at).toLocaleDateString()} ${new Date(camp.scheduled_at).toLocaleTimeString().slice(0,5)}` : new Date(camp.created_at).toLocaleDateString()}
                  </td>
                  <td style={{textAlign:'right'}}>
                      <div style={{display:'flex', gap:15, justifyContent:'flex-end', alignItems:'center'}}>
                        {camp.status === 'scheduled' && (
                            <button onClick={() => handleForceSend(camp)} title="Enviar AGORA (Forçar)" style={{background:'rgba(255, 165, 0, 0.2)', border:'1px solid #FBA94C', color:'#FBA94C', cursor:'pointer', padding:5, borderRadius:4}}>
                                <Play size={16}/>
                            </button>
                        )}
                        {(camp.status === 'draft' || camp.status === 'scheduled') && <button onClick={() => handleEdit(camp)} title="Editar" style={{background:'none', border:'none', color:'#ccc', cursor:'pointer'}}><Edit3 size={18}/></button>}
  ...
  // VIEW DO EDITOR (Tudo igual ao anterior)
                       ...
                       {editorState.audienceType === 'tags' && (
                           <div style={{background:'#202024', padding:10, borderRadius:6}}>
                               {availableTags.map(tag => (
                                   <div key={tag.id} onClick={() => { const newTags = editorState.tags.includes(tag.name) ? editorState.tags.filter(t => t !== tag.name) : [...editorState.tags, tag.name]; setEditorState({...editorState, tags: newTags}); }} style={{display:'inline-block', margin:3, padding:'4px 8px', fontSize:11, borderRadius:4, cursor:'pointer', background: editorState.tags.includes(tag.name) ? '#8257e6' : '#323238', color:'white'}}>{tag.name}</div>
                               ))}
                           </div>
                       )}
                   </div>
          ...
          <div style={{ width: previewDevice === 'mobile' ? 375 : 600, background: 'white', minHeight: 800, transition: 'width 0.3s', boxShadow: '0 0 30px rgba(0,0,0,0.5)', position: 'relative' }}>
             {editorState.blocks.map((block) => (
                 <div key={block.id} onClick={() => { setSelectedBlockId(block.id); setActiveTab('blocks'); }} style={{ position: 'relative', outline: selectedBlockId === block.id ? '2px solid #8257e6' : '1px dashed transparent', cursor: 'pointer' }}>
                   ...
                   {selectedBlockId === block.id && block.type === 'text' ? (
                       <div style={{padding: block.style.padding || 20, background: block.style.backgroundColor}}>
                           <EditableArea html={block.content.text} onChange={(val) => updateBlock(block.id, 'text', val)} style={{fontFamily: block.style.fontFamily || 'sans-serif', fontSize: block.style.fontSize, color: block.style.color, textAlign: block.style.align}}/>
                       </div>
               ...
               {editorState.blocks.length > 0 && (
                   <div style={{padding:20, color:'#999', fontSize:11, textAlign:'center', borderTop:'1px solid #eee', marginTop:20}}>Enviado via Framework Workspace</div>
               )}
          </div>
        </div>

        <div style={{ width: 300, background: '#121214', borderLeft: '1px solid #29292e', padding: 20, overflowY: 'auto' }}>
           <p style={styles.sectionTitle}>EDITAR BLOCO</p>
           {currentBlock ? (
             <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                {/* ... RESTO DAS PROPRIEDADES (BOTÃO, SOCIAL, ETC) PERMANECEM IGUAIS ... */}
      ...
      {isScheduleModalOpen && (
        <div className="modal-overlay" style={{position:'fixed', top:0, left:0, width:'100%', height:'100%', background:'rgba(0,0,0,0.8)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:999}}>
            <div style={{background:'#202024', padding:30, borderRadius:8, width:400, border:'1px solid #323238'}}>
                ...
                <p style={{color:'#a8a8b3', fontSize:12}}>O agendamento automático requer configuração de servidor. Caso não funcione, use o botão de "Enviar Agora" na lista.</p>
